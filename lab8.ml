...
unregisters the listener with that id from he event if there is
one. If there is no listener with that id, do nothing.
......................................................................*)
            
  let remove_listener (evt : 'a event) (i : id) : unit =
    let rec remove_in (vis: 'a waiter list) (back: 'a waiter list) : unit =
      match back with
      | [] -> () 
      | h :: t ->
        if h.id = i then evt := (vis @ t)
        else remove_in (vis @ [h]) t
    in
    remove_in [] !evt ;;
